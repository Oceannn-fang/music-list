...
def parse_end_time(time_str):
    ...
    # 找到中间的空格+时间+横杠的模式
    try:
        # 提取结束时间部分 (2025-01-23 18:00)
        parts = time_str.split('-')
        if len(parts) >= 3:
            # 结束时间由最后两部分组成
            end_time_str = parts[-2] + '-' + parts[-1]
...
def parse_start_time(time_str):
    ...
    try:
        ...
        parts = time_str.split('-')
        if len(parts) >= 3:
            start_time_str = parts[0] + '-' + parts[1] + '-' + parts[2].split(' ')[0] + ' ' + parts[2].split(' ')[1]
...
# 保存验证结果
df.to_csv('C:/Users/86185/.openclaw/workspace/verification_result2.csv', index=False, encoding='utf-8-sig')
print("\n验证结果已保存到 verification_result2.csv")
